...
void writeBuffer(FILE *file, GoBackNMessageStruct *packet) {
    ...
    if ((retval = fwrite(packet->data, 1, count, file)) < count) {
        if (ferror(file)) {
            ...
            fprintf(stderr,
                    "WARNING: Could not write complete packet to file!\n");
        }
    }
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        // DEBUG
        printf('\nexpected seqNo: ', lastReceivedSeqNo + 1);
        printf('\ngot seqNo: ', data->seqNo);
        printf('\ngoodBytes: ', goodBytes);
        ...
        // an older one
        if (data->seqNo < lastReceivedSeqNo + 1) sendAck(s, data, lastReceivedSeqNo + 1);  // TODO might be wrong

        // too new (we missed at least one packet)
        if (data->seqNo > lastReceivedSeqNo + 1) sendAck(s, data, lastReceivedSeqNo + 1);

        // the one we are expecting
        if (data->seqNo == lastReceivedSeqNo + 1) {
            // If this was the last packet of the transmission, close the file
          if (data->size == sizeof(*data) {
                writeBuffer(output, data);
                fclose(output);
                printf('totalBytes: ', totalBytes);
                printf('goodBytes: ', goodBytes);
          } else {
